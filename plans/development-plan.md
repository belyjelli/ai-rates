...
>     1. **A1 — flat max, zero new requests.** Capture `maxLeverage` already present in the bulk responses (Bybit, Gate, KuCoin, Hyperliquid) into a nullable `markets.max_leverage`. Pair page shows `2 × size / min(long, short)` labelled "at small size", `–` where unknown. *Check:* capital changes on a BTC pair and stays honest where a venue is silent.
>     2. **B1 — one venue end to end.** Migration + tier table + Bybit's enumerated ladder + IMR capital on the pair page for Bybit legs only. *Check:* an integration test in `data.int.test.ts` (production client options) and a hand-computed tier boundary — $250k vs $2M on BTCUSDT must give different capital.
