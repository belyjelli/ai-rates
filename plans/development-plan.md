...
>   - Full plan: `~/.claude/plans/whimsical-juggling-dongarra.md`. **Note:** it calls the stat-window migration `006_stat_windows.sql`, but 006 is now `max_leverage` (applied in production). The heatmap migration must be **007**; migrations are applied in filename order and recorded by filename, so a reused number would be skipped as already-applied and the columns would silently never exist.
