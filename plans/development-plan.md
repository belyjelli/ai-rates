...
> - **A price series is also missing in practice.** `funding_snapshots` has a 30-day retention policy but currently spans **0.6 days** (4.57M rows, ~860 samples/24h for bybit BTCUSDT, about one per 100s). Collection started 2026-09-04, so the ~7 missing days need explaining before anyone trusts an event-study window. Aligning liquidations to price moves needs either that series repaired or proper klines.
