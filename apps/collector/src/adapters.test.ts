...
describe("collector adapters", () => {
  ...
  test("still-deferred venues are not collected", () => {
    ...
    for (const deferred of ["txflow", "edgex", "ethereal", "blofin"])
    ...
    expect(ids).toContain("ondo");
    expect(ids).toContain("sodex");
    expect(ids).toContain("standx");
    expect(ids).toContain("coinw");
    expect(ids).toContain("toobit");
    ...
    expect(ids).toContain("nado");
    expect(ids).toContain("polymarket");
    expect(ids).toContain("risex");
    expect(ids).toContain("pacifica");
